...
```
go install github.com/dezeroku/ses_local_email/s3_monitor@v0.5.1 #SED_RELEASE_VERSION
ses_local_email
```
...
```
docker run ghcr.io/dezeroku/ses_local_email_s3_monitor:v0.5.1 #SED_RELEASE_VERSION
```
...
```
go install github.com/dezeroku/ses_local_email_eml_unpack_attachments@v0.5.1 #SED_RELEASE_VERSION
ses_local_email
```
...
```
docker run ghcr.io/dezeroku/ses_local_email_eml_unpack_attachments:v0.5.1 #SED_RELEASE_VERSION
```
...
Github action is responsible for creating a draft release that includes necessary packages (e.g. it's responsible for packaging Lambdas source code).
To prepare a release:

- make sure that you are checked out on the main branch and up-to-date with upstream
- run `scripts/prepare_release <tag name>`, where `<tag name>` must follow `v*.*.*` (usual semver) to create a release commit
- push the changes (both to the branch and new tag)
